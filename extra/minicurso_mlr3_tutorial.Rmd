...
```{r}
tsk_norm <- TaskClassif$new(id="kirc_raw", 
                           backend = kirc_data, 
                           target = "metastasis", 
                           positive = "M1")
...
kirc_bc = p_bc$train(list(tsk_norm))$output$data()

kirc_sr = p_sr$train(list(tsk_norm))$output$data()

kirc_sm = p_sm$train(list(tsk_norm))$output$data()

tsk_bc <- TaskClassif$new(id="kirc_boxcox", 
                           backend = kirc_bc, 
                           target = "metastasis", 
                           positive = "M1")

tsk_sr <- TaskClassif$new(id="kirc_scalerange", 
                           backend = kirc_sr, 
                           target = "metastasis", 
                           positive = "M1")

tsk_sm <- TaskClassif$new(id="kirc_scalemaxabs", 
                           backend = kirc_sm, 
                           target = "metastasis", 
                           positive = "M1")
...
```{r}
learners = c("classif.rpart", "classif.ranger", "classif.xgboost", "classif.kknn")
...
grid = benchmark_grid(
  task = list(tsk_norm, tsk_bc, tsk_sr, tsk_sm),
  learner = learners,
...
cols <- c("task_id", "learner_id", "classif.acc","classif.bacc", "classif.sensitivity", "classif.specificity")
...
tsk_rpart <- TaskClassif$new(id="kirc_rpart", 
                               backend = kirc_bc, 
                               target = "metastasis", 
                               positive = "M1")
...
dd <- kirc_bc
dd$metastasis <- factor(dd$metastasis, ordered = T, levels = c("M0", "M1"))
dd <- mRMR.data(data = dd)

set.seed(1)
fs <- mRMR.ensemble(data = dd, target_indices = 2, feature_count = 20, solution_count = 1)

cols.mRMR <- fs@feature_names[fs@filters[["2"]]]
  
tsk_cla_mRMR <- TaskClassif$new(id="kirc_mRMR", 
                               backend = kirc_bc, 
                               target = "metastasis", 
                               positive = "M1")

tsk_cla_mRMR$select(cols = cols.mRMR)
tsk_cla_mRMR
...
tsk_cla_rfe <- TaskClassif$new(id="kirc_rfe", 
                               backend = kirc_bc, 
                               target = "metastasis", 
                               positive = "M1")
...
tsk_cla_rfe$select(cols = instance$result_feature_set)
tsk_cla_rfe
```
...
grid = benchmark_grid(
  task = list(tsk_rpart, tsk_cla_mRMR, tsk_cla_rfe),
...
```{r}
bmr = benchmark(grid, store_models = TRUE)
bmr$aggregate(msrs(c( "classif.bacc",  "classif.acc", "classif.ce")))
```
...
tsk_cla <- TaskClassif$new(id="kirc_boxcox_metas", 
                               backend = kirc_bc, 
                               target = "metastasis", 
                               positive = "M1")
